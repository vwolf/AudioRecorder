...
public class WaveformAnalyzer {
    ...
    /// Returns the calculated waveform of the initialized URL
    public func samples(count: Int, qos: DispatchQoS.QoSClass = .userInitiated, completionHandler: @escaping (_
        amplitudes: [Float]?) -> ()) {
        waveformSamples(count: count, qos: qos, fftBands: nil) { analysis in
            completionHandler(analysis?.amplitude)
        }
    }
}
    // MARK: Private
...
fileprivate extension WaveformAnalyzer {
    // everything below -50 db will be clipped
    private var silenceDbThreshold: Float { return -50.0 }
    ...
    func extract(totalSamples: Int,
        ...
        // read upfront to avoid frequent re-caluculation (and memory bloat from C-bridging)
        let samplesPerPixel = max(1, totalSamples / targetSampleCount)
        // ~100ms at 44.1kHz, rounded to closest pow(2) for FFT
        let samplesPerFFT = 4096
        ...
        return WaveformAnalysis(amplitude: normalize(outputSamples), fft: outputFFT)
    }
    ...
    private func process(_ sampleBuffer: Data,
        ...
        sampleBuffer.withUnsafeBytes { (samplesRawPointer: UnsafeRawBufferPointer) in
            ...
            // maximum amplitude storable in Int16 = 0db (loudest)
            var zeroDbEquivalent: Float = Float(Int16.max)
            let samplesToProcess = vDSP_Length(sampleLength)
            
            var processingBuffer = [Float](repeating: 0.0, count: Int(samplesToProcess))
            // converts 16 bit into float
            ...
            let downSampleLength = sampleLength / samplesPerPixel
            downSampleData = [Float](repeating: 0.0, count: downSampleLength)
            
            vDSP_desamp(processingBuffer,
                        vDSP_Stride(samplesPerPixel),
                        filter,
                        &downSampleData,
                        vDSP_Length(downSampleLength),
    ...
    private func totalSamplesOfTrack() -> Int {
        ...
        autoreleasepool {
            ...
            descriptions.forEach { formateDescription in
                guard let basicDescription = CMAudioFormatDescriptionGetStreamBasicDescription(formateDescription) else {
                    return
                }
